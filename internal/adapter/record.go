// Package adapter defines the miseledger.adapter.v1 record contract that
// notcrawl emits via `notcrawl export adapter`. It is a self-contained copy of
// the shared contract (the same struct shape stationtrail, sourceharvest,
// discrawl, slacrawl, and graincrawl carry) so MiseLedger can ingest notcrawl
// output without a shared dependency.
package adapter
